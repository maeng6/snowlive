...
/// 준비 중 안내만 띄운다.
///
/// showDialog를 쓰지 않는 이유: 그러면 가장 가까운 Navigator(= WebAppShell 안쪽의
/// 라우트 Navigator)에 붙어서 딤이 콘텐츠 영역만 덮고 GNB/상단바는 밝게 남는다.
/// 중고거래 이미지 뷰어와 동일하게 main_web.dart의 최상위 Overlay를 rootOverlay로
/// 찾아 엔트리를 꽂아서 화면 전체를 덮는다.
Future<void> showRankingProfileModal(BuildContext context, RankingUser user) {
  ...
  entry = OverlayEntry(
    builder: (_) => Positioned.fill(
      child: _RankingProfileModal(user: user, onClose: close),
    ),
  );
...
class _RankingProfileModal extends StatelessWidget {
  final RankingUser user;
  final VoidCallback onClose;

  const _RankingProfileModal({required this.user, required this.onClose});
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: GestureDetector(
          // 카드 자체를 탭했을 때는 닫히지 않도록 이벤트 전파 차단
          onTap: () {},
          child: SingleChildScrollView(
            // 카드 표면을 Material로 만든다. Overlay에 직접 꽂는 구조라 showDialog의
            // Dialog가 주던 Material 조상이 없고, 그러면 안쪽 InkWell이
            // "No Material widget found"로 죽는다. 흰 배경을 Container에 두고
            // MaterialType.transparency로 감싸는 방식은 잉크가 배경 뒤에 깔려
            // 스플래시가 안 보이므로, Material이 표면을 직접 칠하게 한다.
            child: Material(
