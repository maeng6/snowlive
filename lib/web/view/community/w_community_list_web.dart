...
/// 커뮤니티 목록 본문. 모바일은 카드형, 태블릿·데스크탑은 표형으로 그린다.
class CommunityListWeb extends StatelessWidget {
  const CommunityListWeb({super.key});

  @override
  Widget build(BuildContext context) {
    final vm = Get.find<CommunityListPaginationViewModelWeb>();
    ...
    return Obx(() {
      ...
      if (items.isEmpty) {
        return WebEmptyState(
          message: query.isEmpty ? '아직 게시글이 없어요.' : "'$query' 검색 결과가 없어요.",
        );
      }
  ...
  /// 페이지를 옮기면 목록 상단으로 스크롤을 되돌린다(중고거래와 동일).
  Future<void> _gotoPage(
    BuildContext context,
    CommunityListPaginationViewModelWeb vm,
    int page,
  ) async {
